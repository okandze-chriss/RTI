...
from rti import mail, db
from rti.models import Profil
...
def save_file(form_file):
    ...
    picture_path = os.path.join(current_app.root_path, 'static/upload/excel/', file_fn)
    '''
    output_size = (125, 125)
...
# GET ADMIN AND ACCOUNTING
def get_roles():
    return[
        (role.id, role.libelle) for role in db.session.query(Profil).filter(
            (Profil.libelle == 'ADMIN') | (Profil.libelle == 'COMPTABLE')
        )]


# INSERT MULTIPLE OBJECT ONCE INTO A DATABASE
def insertToDB(objects):
    if objects:
        for obj in objects:
            db.session.add(obj)
        db.session.commit()
    else:
        print("No object is passed !")
...
def apiJson():
    """
    forfait = Forfait.query.filter_by(libelle='Bronze').first()
    forfaitTab = []
    dureeMois = Duree.query.get(1)
    dureeAn = Duree.query.get(2)
    f1 = Forfait(libelle='Golden')
    f2 = Forfait(libelle='Bronze')
    f3 = Forfait(libelle='Safir')

    tgm = Taux(taux_interet=0.05, forfait=f1, duree=dureeMois)
    tga = Taux(taux_interet=0.08, forfait=f1, duree=dureeAn)

    tbm = Taux(taux_interet=0.12, forfait=f2, duree=dureeMois)
    tba = Taux(taux_interet=0.16, forfait=f2, duree=dureeAn)

    tsm = Taux(taux_interet=0.2, forfait=f3, duree=dureeMois)
    tsa = Taux(taux_interet=0.25, forfait=f3, duree=dureeAn)

    insertToDB([tgm, tga, tbm, tba, tsm, tsa])

    for forf in forfait.taux:
        d = Duree.query.get(forf.duree_id)
        f = Forfait.query.get(forf.duree_id)


        myobject = {
            'id': forf.id,
            'taux_interet': forf.taux_interet,
            'duree': {'id': d.id, 'libelle': d.libelle},
            #'forfait': {'id': f.id, 'libelle': f.libelle}
        }
        forfaitTab.append(myobject)

        exit(1)
        print(Duree.query.get(forf.duree_id))
        """
    """return jsonify({
        forfait.libelle: {
            'Montant inferieur': 10000,
            'Montant supperieur': 50000
        },
        'forfait.libelle': {
            'Montant inferieur': 10000,
            'Montant supperieur': 50000
